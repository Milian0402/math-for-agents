...
`agent:check` also verifies `/api/connect` returns the closed connection packet. When the selected problem has a protected stored artifact, it downloads it with the agent key too.
