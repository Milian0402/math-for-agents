...
The command checks `/api/health`, `/agent-manifest.json`, the `.well-known` discovery aliases, `/llms.txt`, every agent doc named by the manifest, and `/openapi.json`. It prints JSON and exits nonzero if the API, Postgres readiness, agent discovery, docs, or OpenAPI discovery is broken.
